...
$RecAuthorityC = mysql_query($query_limit_RecAuthorityC, $connect2data) or die(mysql_error());
...
if (isset($_GET['totalRows_RecAuthorityC'])) {
  ...
  $all_RecAuthorityC = mysql_query($query_RecAuthorityC);
  $totalRows_RecAuthorityC = mysql_num_rows($all_RecAuthorityC);
}
...
<!-------顯示頁選擇與分頁設定開始---------->
    <table border="0">
      <tr>
        <td><?php if ($pageNum_RecAuthorityC > 0) { // Show if not first page ?>
        ...
        <?php
			...
			else//$totalPages_RecAuthorityC>10
			{
        ...
        <td><?php if ($pageNum_RecAuthorityC < $totalPages_RecAuthorityC) { // Show if not last page ?>
...
<!-------顯示頁選擇與分頁設定結束---------->
            <td width="110" align="right" class="page_display"><?php if ($totalRows_RecAuthorityC > 0) { // Show if recordset not empty ?>
                頁數:<?php echo (($pageNum_RecAuthorityC+1)."/".($totalPages_RecAuthorityC+1)); ?>
  <?php } // Show if recordset not empty ?>
</td>
            <td width="151" align="right" class="page_display">所有資料數:<?php echo $totalRows_RecAuthorityC ?> </td>
...
//echo $totalRows_RecImage;
		?>
            <td align="center" class="table_data" ><a href="authorityC_edit.php?a_id=<?php echo $row_RecAuthorityC['a_id']; ?>"><?php echo $row_RecAuthorityC['a_title']; ?></a></td>
          ...
          <?php } while ($row_RecAuthorityC = mysql_fetch_assoc($RecAuthorityC)); ?>
        </table>
        <?php } // Show if recordset not empty ?>
</form>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
