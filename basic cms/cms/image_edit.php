...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    $updateSQL = "UPDATE file_set SET file_title=:file_title, file_content=:file_content, file_show_type=:file_show_type, file_width=:file_width, file_height=:file_height WHERE $fileType file_id=:file_id";
    ...
    $sth->bindParam(':file_show_type', $file_show_type, PDO::PARAM_INT);
    $sth->bindParam(':file_width', $file_width, PDO::PARAM_INT);
    $sth->bindParam(':file_height', $file_height, PDO::PARAM_INT);
    ...
    if (isset($_REQUEST['type']) && $_REQUEST['type'] == 'fuckyou') {
        ...
        $image_result = image_process($conn, $_FILES['image'], $_REQUEST['file_title'], "news", "edit", $imagesSize[$format]['IW'], $imagesSize[$format]['IH']);

    } elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'newsCover') {

        $image_result = image_process($conn, $_FILES['image'], $_REQUEST['file_title'], "news", "edit", $IWidth, $IHeight);

    } elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'storeCover') {

        $image_result = image_process($conn, $_FILES['image'], $_REQUEST['file_title'], "store", "edit", $IWidth, $IHeight);
    ...
    for ($j = 1; $j < count($image_result); $j++) {
        $updateSQL = "UPDATE file_set SET file_name=:file_name, file_link1=:file_link1, file_link2=:file_link2, file_link3=:file_link3, file_link4=:file_link4, file_link5=:file_link5, file_show_type=:file_show_type, file_width=:file_width, file_height=:file_height WHERE $fileType file_id=:file_id";
