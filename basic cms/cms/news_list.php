...
$query_Recnews = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_sort ASC, d_date DESC";
$query_limit_Recnews = sprintf("%s LIMIT %d, %d", $query_Recnews, $startRow_Recnews, $maxRows_Recnews);
$Recnews = $conn->query($query_limit_Recnews);
$row_Recnews = $Recnews->fetch();

if (isset($_GET['totalRows'])) {
    $totalRows = $_GET['totalRows'];
} else {
    $all_Recnews = $conn->query($query_Recnews);
    $totalRows = $all_Recnews->rowCount();
}
...
if ($R_pageNum > $totalPages && $R_pageNum != 0) {
    header("Location:news_list.php?pageNum=" . $totalPages);
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Recnews = "SELECT * FROM data_set WHERE d_class1 = 'news' ORDER BY d_sort ASC, d_date DESC";
    $Recnews = $conn->query($query_Recnews);
    $row_Recnews = $Recnews->fetch();

    do {
        if ($row_Recnews['d_sort'] == 0) {} else if ($row_Recnews['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recnews['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Recnews = $Recnews->fetch());
    ...
    if ($G_changeSort == 1) {
        header("Location:news_list.php?pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
    } else if ($G_delchangeSort == 1) {
        header("Location:news_list.php?pageNum=" . $_GET['pageNum']);
    }
}
                            ...
                            <form action="news_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        ...
                                        $query_RecImage = "SELECT * FROM file_set WHERE file_type='newsCover' AND file_d_id = :file_d_id";
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <a href="news_edit.php?d_id=<?php echo $row_Recnews['d_id']; ?>">
                                        ...
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recnews['d_id']; ?>',this.options[this.selectedIndex].value)">
                                                <option value="0" <?php if (!(strcmp(0, $row_Recnews[ 'd_sort']))) {echo "selected"; } ?>>至頂</option>
                                                <?php
                                        ...
                                        <td align="center" class="table_data"><a href="news_edit.php?d_id=<?php echo $row_Recnews['d_id']; ?>">
                                    ...
                                    <?php } while ($row_Recnews = $Recnews->fetch()); ?>
                                </table>
                                <?php } // Show if recordset not empty ?>
                            </form>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
        ...
        window.location.href = "news_list.php?pageNum=" + pageNum + "&totalRows=" + totalRows + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num;
    }
</script>