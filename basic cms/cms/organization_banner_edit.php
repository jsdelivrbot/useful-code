...
$colname_Recorganization_banner = "-1";
if (isset($_GET['d_id'])) {
  $colname_Recorganization_banner = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_Recorganization_banner = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_Recorganization_banner, "int"));
...
$totalRows = mysql_num_rows($Recorganization_banner);
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecCover = sprintf("SELECT * FROM file_set WHERE file_d_id = %s AND file_type = 'organization_bannerCover'", GetSQLValueString($colname_RecImage, "int"));
$RecCover = mysql_query($query_RecCover, $connect2data) or die(mysql_error());
$row_RecCover = mysql_fetch_assoc($RecCover);
$totalRows_RecCover = mysql_num_rows($RecCover);
                ...
                <td>
                  <table width="100%" border="0" cellspacing="3" cellpadding="5">
                   <tr>
                     ...
                     <td width="532"><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_Recorganization_banner['d_title']; ?>" size="50" />
                      <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_Recorganization_banner['d_id']; ?>" /></td>
                      <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                    </tr>

                    <tr>
                     <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
                     <td width="532"><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_Recorganization_banner['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_Recorganization_banner['d_date']))) ) ? date("Y-m-d H:i:s") : $row_Recorganization_banner['d_date']; ?>" size="50"></td>
                     <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                   </tr>
                   <tr>
                     ...
                     <td width="532"><label>
                       <select name="d_active" class="table_data" id="d_active">
                         <option value="1" <?php if (!(strcmp(1, $row_Recorganization_banner['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_Recorganization_banner['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
                       </select></label></td>
                       <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                     </tr>
                     ...
                     <tr>
                      ...
                      <td><?php do { ?>
                        ...
                        <td bgcolor="#e5ecf6" class="table_col_title"><p class="red_letter">*<?php echo $imagesSize['organization_banner']['note'];?></p></td>
                        ...
                        <td>
                          <table width="100%" border="0" cellpadding="2" cellspacing="2" bordercolor="#CCCCCC" class="data" id="pTable">
                            ...
                            <td bgcolor="#e5ecf6" class="table_col_title">
                              <p class="red_letter">*<?php echo $imagesSize['organization_banner']['note'];?></p></td>
                            </tr>
                            <?php } // Show if recordset not empty ?>
...
do {
  ...
  <td><table width="320" border="1" cellpadding="0" cellspacing="0" bordercolor="#666666" class="table_frame_style">
    ...
    <tr>
      <td align="left" class="table_no_border"><a href="organization_banner_file_edit.php?file_id=<?php echo $row_RecFile['file_id']; ?>"><img src="image/media_edit.gif" width="16" height="16" title="修改檔案" /></a><a href="organization_banner_file_del.php?file_id=<?php echo $row_RecFile['file_id']; ?>"><img src="image/media_delete.gif" width="16" height="16" title="刪除檔案"/></a></td>
    </tr>
  </table></td>
  <?php  $RecFile_endRow++;
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  for($j=1;$j<count($image_result);$j++)
  {
    ...
    // Cover
    //一般附圖
  $image_result=image_process($_FILES['imageCover'], $_REQUEST['image_titleCover'], "organization_bannerCover","add", $imagesSize['organization_bannerCover']['IW'], $imagesSize['organization_bannerCover']['IH']);
  //echo count($image_result);
  //echo $image_result[0][0];
  ...
  for($j=1;$j<count($image_result);$j++)
  {
    $insertSQL = sprintf("INSERT INTO file_set (file_name, file_link1, file_link2, file_link3, file_type, file_d_id, file_title, file_show_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
     ...
     GetSQLValueString($image_result[$j][2], "text"),
     GetSQLValueString($image_result[$j][3], "text"),
     GetSQLValueString("organization_bannerCover", "text"),
     GetSQLValueString($_POST['d_id'], "int"),
     GetSQLValueString($image_result[$j][4], "text"),
     GetSQLValueString($image_result[$j][5], "int"));
  ...
  if($ifFile){
   ...
   $file_result=file_process("organization_banner","add");
 ...
 $updateGoTo = "organization_banner_list.php";
 if (isset($_SERVER['QUERY_STRING'])) {
