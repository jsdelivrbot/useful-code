...
$colname_Recorganization = "-1";
if (isset($_GET['d_id'])) {
  $colname_Recorganization = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_Recorganization = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_Recorganization, "int"));
...
$totalRows = mysql_num_rows($Recorganization);
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecCover = sprintf("SELECT * FROM file_set WHERE file_d_id = %s AND file_type = 'organizationCover'", GetSQLValueString($colname_RecImage, "int"));
...
<body>
  <table width="1280" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <td>
                  <table width="100%" border="0" cellspacing="3" cellpadding="5">
                   <tr>
                     ...
                     <td width="532"><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_Recorganization['d_title']; ?>" size="50" />
                      <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_Recorganization['d_id']; ?>" /></td>
                      <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                    </tr>

                    <!-- <tr>
                      <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容</td>
                      <td><textarea name="d_content" cols="50" rows="8" class="table_data" id="d_content"><?php echo $row_Recorganization['d_content']; ?></textarea></td>
                      <td bgcolor="#e5ecf6" class="table_col_title"><p class="note_letter">*小斷行請按Shift+Enter。<br />
                        輸入區域的右下角可以調整輸入空間的大小。</p></td>
                      </tr> -->

                      <tr>
                       <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
                       <td width="532"><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_Recorganization['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_Recorganization['d_date']))) ) ? date("Y-m-d H:i:s") : $row_Recorganization['d_date']; ?>" size="50"></td>
                       <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                     </tr>
                     <tr>
                       ...
                       <td width="532"><label>
                         <select name="d_active" class="table_data" id="d_active">
                           <option value="1" <?php if (!(strcmp(1, $row_Recorganization['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_Recorganization['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
                         </select></label></td>
                         <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                       </tr>
                          ...
                          <td>
                            <table width="100%" border="0" cellpadding="2" cellspacing="2" bordercolor="#CCCCCC" class="data" id="pTable">
                              ...
                              <td bgcolor="#e5ecf6" class="table_col_title">
                                <p class="red_letter">*<?php echo $imagesSize['organization']['note'];?></p></td>
                              </tr> -->
                              <?php } // Show if recordset not empty ?>
    ...
    // Cover
    //一般附圖
          $image_result=image_process($_FILES['imageCover'], $_REQUEST['image_titleCover'], "organizationCover","add", $imagesSize['organizationCover']['IW'], $imagesSize['organizationCover']['IH']);
  //echo count($image_result);
  //echo $image_result[0][0];
          ...
          for($j=1;$j<count($image_result);$j++)
          {
            $insertSQL = sprintf("INSERT INTO file_set (file_name, file_link1, file_link2, file_link3, file_type, file_d_id, file_title, file_show_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
             ...
             GetSQLValueString($image_result[$j][2], "text"),
             GetSQLValueString($image_result[$j][3], "text"),
             GetSQLValueString("organizationCover", "text"),
             GetSQLValueString($_POST['d_id'], "int"),
             GetSQLValueString($image_result[$j][4], "text"),
             GetSQLValueString($image_result[$j][5], "int"));
	    ...
	    //----------插入檔案資料到資料庫begin(須放入插入主資料後)----------
           ...
           $file_result=file_process("organization","add");
	...
	//----------插入檔案資料到資料庫end----------
         ...
         $updateGoTo = "organization_list.php";
         if (isset($_SERVER['QUERY_STRING'])) {
