            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">分類名稱</td>
          	    	<td class="table_data"><?php echo $row_RecroomsT['name']; ?>
...
<?php
    ...
    $sql="SELECT file_link1 FROM file_set WHERE file_type='roomsTCover' AND file_d_id='".$_POST['term_id']."'";
    ...
    $sql="SELECT file_link2 FROM file_set WHERE file_type='roomsTCover' AND file_d_id='".$_POST['term_id']."'";
    $result = mysql_query($sql)or die("無法送出".mysql_error( ));
    while ( $row = mysql_fetch_array($result))
    {
      //echo "../".$row[0]."<BR>";
    ...
    $sql="SELECT file_link3 FROM file_set WHERE file_type='roomsTCover' AND file_d_id='".$_POST['term_id']."'";
    $result = mysql_query($sql)or die("無法送出".mysql_error( ));
    while ( $row = mysql_fetch_array($result))
    {
      //echo "../".$row[0]."<BR>";
    ...
    $sql="SELECT file_link4 FROM file_set WHERE file_type='roomsTCover' AND file_d_id='".$_POST['term_id']."'";
    $result = mysql_query($sql)or die("無法送出".mysql_error( ));
    while ( $row = mysql_fetch_array($result))
    {
      //echo "../".$row[0]."<BR>";
    ...
    $deleteSQL = sprintf("DELETE FROM file_set WHERE file_type='roomsTCover' AND file_d_id='".$_POST['term_id']."'");
