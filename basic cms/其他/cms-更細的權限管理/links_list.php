...
$startRow_RecLinks = $pageNum * $maxRows_RecLinks;

mysql_select_db($database_connect2data, $connect2data);
$query_RecLinks = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_sort ASC, d_date DESC";
...
if (isset($_GET['totalRows'])) {
  ...
  $totalRows = mysql_num_rows($all_RecLinks);
}
$totalPages = ceil($totalRows/$maxRows_RecLinks)-1;
...
<?php
	...
	{
		header("Location:links_list.php?pageNum=".$totalPages);
	}
...
function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
 ...
 window.location.href="links_list.php?pageNum="+pageNum+"&totalRows="+totalRows+"&changeSort=1"+"&now_d_id="+now_d_id+"&change_num="+change_num;
}
//-->
</script>
<!-- InstanceEndEditable -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
    	<tr>
        	<td width="30%" class="list_title">友站連結列表</td>
	...
	<form action="links_process.php" method="post" name="form1" id="form1">
      <?php if ($totalRows > 0) { // Show if recordset not empty ?>
		...
		echo "<tr bgcolor='#E4E4E4'>";}
  		?>
          <td align="center" class="table_data" ><a href="links_edit.php?d_id=<?php echo $row_RecLinks['d_id']; ?>"><?php echo $row_RecLinks['d_date']; ?></a></td>
          <td align="center" class="table_data" ><label>
        <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_RecLinks['d_id']; ?>',this.options[this.selectedIndex].value)">
        ...
        <td align="center" class="table_data" ><a href="links_edit.php?d_id=<?php echo $row_RecLinks['d_id']; ?>"><?php echo $row_RecLinks['d_title']; ?></a></td>
