...
$query_RecShopProcess = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_RecShopProcess, "int"));
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecShopProcessC = "SELECT c_title FROM class_set WHERE c_parent = 'shopProcessC' AND c_id = ".$G_selected1." ORDER BY c_sort ASC, c_id DESC";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">刪除購物說明</td>
            <td width="70%"><span class="no_data">確定刪除以下購物說明?</span></td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
  		...
  		<tr>
            ...
            <table width="100%" border="0" cellpadding="5" cellspacing="3">
            	<tr>
            	  ...
            	  <td class="table_data"><?php echo $row_RecShopProcessC['c_title']; ?>
            	...
            	<tr>
            	  ...
            	  <td width="532" class="table_data"><?php echo $row_RecShopProcess['d_title']; ?></td>
            	  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
          	  </tr>
            	<tr>
            	  <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容說明</td>
            	  <td class="table_data"><?php echo $row_RecShopProcess['d_content']; ?></td>
            	  <td bgcolor="#e5ecf6">&nbsp;</td>
          	  </tr>
                <tr>
                  <td align="center" bgcolor="#e5ecf6" class="table_col_title">時間</td>
                  <td class="table_data"><?php echo $row_RecShopProcess['d_date']; ?></td>
                  <td bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                  <?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
     	      	<tr>
                    <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容圖片</td>
                    <td><?php do { ?>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
...
<?php
	if ((isset($_REQUEST['d_id'])) && ($_REQUEST['d_id'] != "") && (isset($_REQUEST['delsure']))) {
	    ...
	    $sql="SELECT file_link1 FROM file_set WHERE file_type='coverImage' AND file_d_id='".$_POST['d_id']."'";
		...
		$sql="SELECT file_link2 FROM file_set WHERE file_type='coverImage' AND file_d_id='".$_POST['d_id']."'";
		$result = mysql_query($sql)or die("無法送出".mysql_error( ));
		while ( $row = mysql_fetch_array($result))
		{
			//echo "../".$row[0]."<BR>";
		...
		$sql="SELECT file_link3 FROM file_set WHERE file_type='coverImage' AND file_d_id='".$_POST['d_id']."'";
		$result = mysql_query($sql)or die("無法送出".mysql_error( ));
		while ( $row = mysql_fetch_array($result))
		{
			//echo "../".$row[0]."<BR>";
	  ...
	  $deleteSQL = sprintf("DELETE FROM file_set WHERE file_type='coverImage' AND file_d_id=%s",
	  ...
	  $deleteGoTo = "shopProcess_list.php?delchangeSort=1&selected1=".$G_selected1;
