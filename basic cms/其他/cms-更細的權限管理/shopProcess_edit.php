...
$colname_RecShopProcess = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecShopProcess = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecShopProcess = sprintf("SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_id = %s", GetSQLValueString($colname_RecShopProcess, "int"));
...
$totalRows_RecShopProcess = mysql_num_rows($RecShopProcess);

$colname_RecCoverImage = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecCoverImage = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecCoverImage = sprintf("SELECT * FROM file_set WHERE file_d_id = %s AND file_type = 'coverImage'", GetSQLValueString($colname_RecCoverImage, "int"));
$RecCoverImage = mysql_query($query_RecCoverImage, $connect2data) or die(mysql_error());
$row_RecCoverImage = mysql_fetch_assoc($RecCoverImage);
$totalRows_RecCoverImage = mysql_num_rows($RecCoverImage);
...
$G_selected1 = '';
if (isset($_SESSION['selected_shopProcessC'])){
	$G_selected1 = $_SESSION['selected_shopProcessC'] = $row_RecShopProcess['d_class2'];
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改購物說明</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
...
?>
            	...
            	<tr>
            	  ...
            	  <td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecShopProcess['d_title']; ?>" size="50" />
          	    </td>
            	  <td bgcolor="#e5ecf6">                  
            	    <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecShopProcess['d_id']; ?>" />
                    <input name="d_sort" type="hidden" id="d_sort" value="<?php echo $row_RecShopProcess['d_sort']; ?>" /><?php //echo $row_RecUnitary['d_sort']; ?>
                  </td>
          	  </tr>
            	<tr>
            	  <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容說明</td>
            	  <td><textarea name="d_content" cols="50" class="table_data tiny" id="d_content"><?php echo $row_RecShopProcess['d_content']; ?></textarea></td>
            	  <td bgcolor="#e5ecf6">&nbsp;</td>
          	  </tr>
                <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">時間</td>
                  <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo $row_RecShopProcess['d_date']; ?>" size="50" /></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                <tr>
                  ...
                  <td><label>
                    <select name="d_active" class="table_data" id="d_active">
                      <option value="0" <?php if (!(strcmp(0, $row_RecShopProcess['d_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecShopProcess['d_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                    </select>
                  </label></td>
                  <td bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
<?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
                  <tr>
                    ...
                    <td><?php do { ?>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr>
                            <td align="left" class="table_data">&nbsp;圖片路徑： <?php echo $row_RecImage['file_link1']; ?></td>
                              </tr>
                          <tr>
                  ...
                  <tr>
                    ...
                    <ul class="red_letter">
                    	<li><?php echo $imagesSize[$_SESSION['nowMenu']]['note'];?></li>
                    	<li><?php echo $maxFileSize; ?>若超過請分批上傳。</li>
                  	</ul>
                    </td>
                </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 if(isset($_POST['select1'])){
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
	...
	$image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "shopProcess","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
	...
	//----------插入檔案資料到資料庫begin(須放入插入主資料後)----------
			...
			$file_result=file_process("shopProcess","add");
  ...
  $_SESSION['original_selected']=$_SESSION['selected_shopProcess'];
  
  $updateGoTo = "shopProcess_list.php?selected1=".$class2."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$_POST['d_id']."&totalRows_RecShopProcess=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
