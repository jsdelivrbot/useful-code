...
$startRow_RecAbout = $pageNum * $maxRows_RecAbout;
...
/*if (isset($_GET['totalRows'])) {
  ...
  $totalRows = mysql_num_rows($all_RecAbout);
}*/
$all_RecAbout = mysql_query($query_RecAbout);
$totalRows = mysql_num_rows($all_RecAbout);
$totalPages = ceil($totalRows/$maxRows_RecAbout)-1;
...
<?php
	...
	{
		header("Location:about_list.php?pageNum=".$totalPages);
	}
...
if ($G_changeSort==1||$G_delchangeSort==1) 
{
	...
	if($G_changeSort==1)
	{
	header("Location:about_list.php?pageNum=".$_GET['pageNum']."&totalRows=".$_GET['totalRows']);
	}else if($G_delchangeSort==1)
	{
	header("Location:about_list.php?pageNum=".$_GET['pageNum']);
	}
}
$_SESSION['totalRows']=$totalRows;
?>
...
function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
 ...
 window.location.href="about_list.php?pageNum="+pageNum+"&totalRows="+totalRows+"&changeSort=1"+"&now_d_id="+now_d_id+"&change_num="+change_num;
}
//-->
</script>
<!-- InstanceEndEditable -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
    	<tr>
        	<td width="30%" class="list_title">創辦人簡介列表</td>
  ...
  <?php } // Show if recordset not empty ?>
      ...
      <?php if ($totalRows > 0) { // Show if recordset not empty ?>
        <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
          <tr>
            ...
            <?php if(0 && in_array(5,$_SESSION['MM_Limit']['a3'])){ ?>
		...
		?>
          ...
          if(in_array(5,$_SESSION['MM_Limit']['a3'])){
			  ?>
           <label>
        <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_RecAbout['d_id']; ?>',this.options[this.selectedIndex].value)">
		...
		<?php }else{
			if (!(strcmp(0, $row_RecAbout['d_sort']))) {
				echo "至頂";
			}else{
				echo $row_RecAbout['d_sort'];
			}
    ...
    <?php } ?>
      ...
      ?>
        ...
        <?php
          if(in_array(5,$_SESSION['MM_Limit']['a3'])){
        echo '<a href="about_edit.php?d_id='.$row_RecAbout['d_id'].'">'.$row_RecAbout['d_title_en'].'</a>';
      }else{
        echo $row_RecAbout['d_title_en'];
      }
      ?>
        ...
        <td align="center"  class="table_data">
            ...
            <?php //發佈狀態
          if(in_array(5,$_SESSION['MM_Limit']['a3'])){
        if($row_RecAbout['d_pub']){
          //echo "<a href='".$row_RecAbout['d_pub']."' rel='".$row_RecAbout['d_id']."' class='pubD' title='發佈'><img src=\"image/accept.png\" width=\"16\" height=\"16\"  ></a>";
          echo "<a href='".$row_RecAbout['d_pub']."' rel='".$row_RecAbout['d_id']."' class='pubD' title='發佈'>發佈</a>";
        }else{
          //echo "<a href='".$row_RecAbout['d_pub']."' rel='".$row_RecAbout['d_id']."' class='pubD' title='草稿'><img src=\"image/delete.png\" width=\"16\" height=\"16\"  ></a>";

          echo "<a href='".$row_RecAbout['d_pub']."' rel='".$row_RecAbout['d_id']."' class='pubD' title='草稿'>草稿</a>";
        }
      }else{
        if($row_RecAbout['d_pub']){
          echo "發佈";
      ...
      ?>
          ...
          <?php if(in_array(5,$_SESSION['MM_Limit']['a3'])){ ?>
            ...
            <?php } ?>
            <?php if(0 && in_array(7,$_SESSION['MM_Limit']['a3'])){ ?>
            <td align="center" class="table_data"><a href="about_del.php?d_id=<?php echo $row_RecAbout['d_id']; ?>"><img src="image/cross.png" width="16" height="16" /></a></td>
            <?php } ?>
          </tr>
