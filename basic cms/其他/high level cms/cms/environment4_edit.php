...
if(!in_array(5,$_SESSION['MM_Limit']['a12'])){
	header("Location: environment4_list.php");
}
...
$colname_RecEnvironment4 = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecEnvironment4 = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecEnvironment4 = sprintf("SELECT * FROM data_set WHERE d_class1 = 'environment4' AND d_id = %s", GetSQLValueString($colname_RecEnvironment4, "int"));
$RecEnvironment4 = mysql_query($query_RecEnvironment4, $connect2data) or die(mysql_error());
$row_RecEnvironment4 = mysql_fetch_assoc($RecEnvironment4);
$totalRows = mysql_num_rows($RecEnvironment4);
...
$colname_RecImage = "-1";
if (isset($row_RecEnvironment4['d_id'])) {
  $colname_RecImage = $row_RecEnvironment4['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
...
if (isset($row_RecEnvironment4['d_id'])) {
  $colname_RecImageCover = $row_RecEnvironment4['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
...
$colname_RecFile = "-1";
if (isset($row_RecEnvironment4['d_id'])) {
  $colname_RecFile = $row_RecEnvironment4['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecBranch = "SELECT * FROM data_set WHERE d_class1 = 'branch' AND d_active='1' ORDER BY d_sort ASC, d_date DESC";
$RecBranch = mysql_query($query_RecBranch, $connect2data) or die(mysql_error());
$row_RecBranch = mysql_fetch_assoc($RecBranch);
$totalRows_RecBranch = mysql_num_rows($RecBranch);

//echo 'selected_Branch = '.$_SESSION['selected_Branch'];
/*$G_sel = '';
if (isset($_SESSION['selected_Branch'])){
  $G_sel = $_SESSION['selected_Branch'] = $row_RecEnvironment4['d_class2'];
  //echo 'G_sel = '.$G_sel;
}*/
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    //echo "d_price1 = ".$d_price1."<br>";
    $checkCoverFormat   = checkV('checkCoverFormat');
    //echo "checkCoverFormat = ".$checkCoverFormat."<br>";
    if(!isset($d_price1) || $d_price1==""){
    $d_price1   = $checkCoverFormat;
    }

    //echo "d_price1 = ".$d_price1."<br>";


  $updateSQL = sprintf("UPDATE data_set SET d_title=%s, d_title_en=%s, d_content=%s, d_price1=%s, d_class2=%s, d_class3=%s, d_class4=%s, d_class6=%s, d_data1=%s, d_data2=%s, d_data3=%s, d_data4=%s, d_date=%s, d_active=%s, d_pub=%s WHERE d_id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
    ...
    $image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "environment4","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);  
    ...
    $image_result = image_process($_FILES['imageCover'], $_REQUEST['imageCover_title'], $_SESSION['nowMenu'].'Cover',"add", $imagesSize[$_SESSION['nowMenu'].'Cover_'.$d_price1]['IW'], $imagesSize[$_SESSION['nowMenu'].'Cover_'.$d_price1]['IH']);
    ...
    if($ifFile){
      ...
      $file_result=file_process("environment4","add");
  ...
  $_SESSION['original_selected']=$_SESSION['selected_Branch'];

  mysql_select_db($database_connect2data, $connect2data);
  //$query_RecEnvironment4 = "SELECT * FROM data_set WHERE d_class1 = 'environment4' ORDER BY d_sort ASC, d_date DESC";
  $query_RecEnvironment4 = "SELECT * FROM data_set WHERE d_class2='$G_sel' AND d_class1='environment4' AND d_active='1' ORDER BY d_sort ASC, d_date DESC";
  $RecEnvironment4 = mysql_query($query_RecEnvironment4, $connect2data) or die(mysql_error());
  $totalRows = mysql_num_rows($RecEnvironment4);

  $_SESSION['totalRows'] = $totalRows;

  $stringLink = "?sel=".$d_class2."&now_d_id=".$d_id."&totalRows_RecOwner=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];

  $updateGoTo = "environment4_list.php".$stringLink
  ;
  if (isset($_SERVER['QUERY_STRING'])) {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改五校環境美學</td>
            <td width="70%">&nbsp;
            </td>
        </tr>
	</table>
    ...
    <div class="groupBtn">
      ...
      //$reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/environment3_1.php?y=".$row_RecEnvironment4['d_class2']."&m=".$row_RecEnvironment4['d_class3'];

     $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."environment3_1.php?sel=".$row_RecEnvironment4['d_class2'];
     /* if( (isset($row_RecEnvironment4['d_class2'])&&$row_RecEnvironment4['d_class2']!='') ){
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."environment3_1.php?sel=".$row_RecEnvironment4['d_class2'];
      }else{
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."share.php";
      }*/
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
  		<tr>
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	...
            	<tr>
               <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">分校</td>
               <td>
                  <select name="d_class2" id="d_class2" class="table_data">
                     <?php
                       do {  
                       ?>
                        <option value="<?php echo $row_RecBranch['d_id']?>" <?php if (!(strcmp($row_RecBranch[ 'd_id'], $row_RecEnvironment4[ 'd_class2']))) {echo "selected='selected'";} ?>><?php echo $row_RecBranch['d_title_en']?></option>
                        <?php
                         } while ($row_RecBranch = mysql_fetch_assoc($RecBranch));
                           $rows = mysql_num_rows($RecBranch);
                           if($rows > 0) {
                               mysql_data_seek($RecBranch, 0);
                             $row_RecBranch = mysql_fetch_assoc($RecBranch);
                           }
                         ?>
                  </select>
              ...
              <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">標題</td>
          	    	<td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecEnvironment4['d_title']; ?>" size="50" />
          	    	  <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecEnvironment4['d_id']; ?>" /></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	    	</tr>
              ...
              <tr>
                  ...
                  <td>
                    <textarea name="d_data1" cols="100" rows="20" class="table_data" id="d_data1"><?php echo $row_RecEnvironment4['d_data1']; ?></textarea>
                  </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
              ...
              <tr>
                  ...
                  <td>
                    ...
                    <textarea name="d_content" cols="100" rows="20" class="table_data" id="d_content"><?php echo $row_RecEnvironment4['d_content']; ?></textarea>
                  </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
                ...
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
          	    	<td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_RecEnvironment4['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_RecEnvironment4['d_date']))) ) ? date("Y-m-d H:i:s") : $row_RecEnvironment4['d_date']; ?>" size="50"></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_active" class="table_data" id="d_active">
                      <option value="1" <?php if (!(strcmp(1, $row_RecEnvironment4['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_RecEnvironment4['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecEnvironment4['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option><option value="0" <?php if (!(strcmp(0, $row_RecEnvironment4['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <tr>
                   <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">目前封面圖片</td>
                   <td>
                      <?php do { ?>
                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                         <tr>
                         ...
                         <tr>
                            <td align="left" class="table_data">
                               <?php if(0){ ?>&nbsp;圖片連結：<?php //echo $row_RecImageCover['file_link1']; ?><?php } ?>
                            </td>
                         </tr>
                         <tr>
                            <td align="center">
                               <a href="image_edit.php?type=imageCoverFormat&formatType=<?php echo $row_RecImageCover['file_show_type'];?>&file_id=<?php echo $row_RecImageCover['file_id']; ?>" class="fancyboxEdit" title="修改圖片"><img src="image/media_edit.gif" width="16" height="16" title="修改圖片" /></a>
                               <a href="image_del.php?type=imageCoverFormat&formatType=<?php echo $row_RecImageCover['file_show_type'];?>&file_id=<?php echo $row_RecImageCover['file_id']; ?>" class="fancyboxEdit" title="刪除圖片"><img src="image/media_delete.gif" width="16" height="16" title="刪除圖片" /></a>
                            </td>
                            <td align="center">&nbsp;</td>
                         </tr>
                      </table>
                      <?php 
                    } while ($row_RecImageCover = mysql_fetch_assoc($RecImageCover));
                    $rows = mysql_num_rows($RecImageCover);
                    if($rows > 0) {
                      mysql_data_seek($RecImageCover, 0);
                      $row_RecImageCover = mysql_fetch_assoc($RecImageCover);
                    }
                    ?>
                   </td>
                         ...
                         <tr>
                            <td> <span class="table_data">選擇圖片：</span>
                               ...
                               <input name="imageCover_title[]" type="text" class="table_data" id="imageCover_title1">
                                <br><span class="table_data">封面樣式：</span><br>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td width="100" rowspan="2" align="center"><a href="../<?php echo $row_RecImage['file_link1']; ?>" class="fancyboxImg" rel="group" title="<?php echo $row_RecImage['file_title']; ?>"><img src="../<?php echo $row_RecImage['file_link2']; ?>" alt="" class="image_frame"/></a></td>
                                    <td align="left" class="table_data"><div class="ml">&nbsp;圖片說明：<?php echo $row_RecImage['file_title']; ?></div></td>
                              </tr>
                          <tr>
...
do {
   ...
   ?>
                                    <td><table width="320" border="1" cellpadding="0" cellspacing="0" bordercolor="#666666" class="table_frame_style">
                                        ...
                                        <tr>
                                          <td align="left" class="table_no_border"><a href="environment4_file_edit.php?file_id=<?php echo $row_RecFile['file_id']; ?>"><img src="image/media_edit.gif" width="16" height="16" title="修改檔案" /></a><a href="environment4_file_del.php?file_id=<?php echo $row_RecFile['file_id']; ?>"><img src="image/media_delete.gif" width="16" height="16" title="刪除檔案"/></a></td>
                                        </tr>
                                    </table></td>
                                    <?php  $RecFile_endRow++;
...
echo("</tr>");
    ...
    <input name="checkCoverFormat" type="hidden" id="checkCoverFormat" value="<?php echo $row_RecImageCover['file_show_type'];?>" />
    <input type="hidden" name="d_class4" id="d_class4" value="<?php echo $row_RecShare['d_class4']; ?>" />
    </form>
...
<?php
mysql_free_result($RecEnvironment4);
