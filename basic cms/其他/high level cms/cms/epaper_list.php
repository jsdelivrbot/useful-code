...
if(isset($_REQUEST['start']) && isset($_REQUEST['end'])){
	...
	if($start!='' && $end!=''){
		...
		$dateSQL = " AND (`e_date` BETWEEN '".$start." 00:00:00' AND '".$end." 23:59:59')";
	}	
	//echo "以日期查詢<br>";
}
...
if(isset($_REQUEST['fulltxt']) && $_REQUEST['fulltxt']!=''){
	...
	$fulltxtSQL = " AND e_title LIKE '%".$fulltxt."%'";
	//echo "以文字搜尋<br>";
}

mysql_select_db($database_connect2data, $connect2data);
$query_RecEpaper = "SELECT * FROM epaper_set WHERE e_class1='epaper' $dateSQL $fulltxtSQL ORDER BY e_date DESC, e_id ASC";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td width="200" class="list_title">電子報EMAIL列表</td>
          <td align="right">
          ...
          <?php if ($totalRows_RecEpaper == 0) { // Show if recordset empty ?>
            <span class="no_data"><strong>目前沒有任何電子報EMAIL資料</strong></span>&nbsp;&nbsp;
...
<form action="epaper_process.php" method="post" name="form1" id="form1">
  <?php if ($totalRows_RecEpaper > 0) { // Show if recordset not empty ?>
    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
      ...
      <td width="142" align="center" class="table_title">日期</td>
        <td align="center" class="table_title">EMAIL</td>
  ...
  <?php
		...
		$colname_RecImage = "-1";
		if (isset($row_RecEpaper['e_id'])) {
		  $colname_RecImage = $row_RecEpaper['e_id'];
		}
		mysql_select_db($database_connect2data, $connect2data);
		...
		<td align="center" class="table_data" >
          ...
          if(in_array(5,$_SESSION['MM_Limit']['a10'])){
			  echo '<a href="epaper_edit.php?d_id='.$row_RecEpaper['e_id'].'">'.$row_RecEpaper['e_date'].'</a>';
		  }else{
			  echo $row_RecEpaper['e_date'];
		  }
		  ?>
          ...
          <?php
          if(in_array(5,$_SESSION['MM_Limit']['a10'])){
			  echo '<a href="epaper_edit.php?e_id='.$row_RecEpaper['e_id'].'">'.$row_RecEpaper['e_title'].'</a>';
		  }else{
			  echo $row_RecEpaper['e_title'];
		  }
		  ?>
          
          <!--<a href="epaper_edit.php?e_id=<?php echo $row_RecEpaper['e_id']; ?>"><?php echo $row_RecEpaper['e_title']; ?></a>-->
		  ...
		  <?php
          if(in_array(5,$_SESSION['MM_Limit']['a10'])){
			  if($row_RecEpaper['e_active']){
					echo "<a href='".$row_RecEpaper['e_active']."' rel='".$row_RecEpaper['e_id']."' class='activeChE'><img src=\"image/accept.png\" width=\"16\" height=\"16\"  ></a>";
				}else{
					echo "<a href='".$row_RecEpaper['e_active']."' rel='".$row_RecEpaper['e_id']."' class='activeChE'><img src=\"image/delete.png\" width=\"16\" height=\"16\"  ></a>";
				}
		  }else{
			  if($row_RecEpaper['e_active']){
		  ...
		  <?php  //list使用
				/*if($row_RecEpaper['e_active']=="1")
				{
					echo "<a href='".$row_RecEpaper['e_active']."' rel='".$row_RecEpaper['e_id']."' class='activeChM'><img src=\"image/accept.png\" width=\"16\" height=\"16\"  ></a>";
				}
				else if($row_RecEpaper['e_active']=="0")
				{
					echo "<a href='".$row_RecEpaper['e_active']."' rel='".$row_RecEpaper['e_id']."' class='activeChM'><img src=\"image/delete.png\" width=\"16\" height=\"16\"  ></a>";
				}else
				{
					echo "未驗證";
				}*/
