...
if (isset($_GET['term_id'])) {
  $colname_RecEventT = $_GET['term_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecEventT = sprintf("SELECT * FROM terms WHERE term_id = %s", GetSQLValueString($colname_RecEventT, "int"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改活動訊息分類</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="3" cellpadding="5">
            <tr>
              ...
              <td width="516"><input name="name" type="text" class="table_data" id="name" value="<?php echo $row_RecEventT['name']; ?>" size="50" />
                  <input name="term_id" type="hidden" id="term_id" value="<?php echo $row_RecEventT['term_id']; ?>" /></td>
            ...
            <tr>
              ...
              <td width="516"><label>
                <select name="term_active" class="table_data" id="term_active">
                  <option value="0" <?php if (!(strcmp(0, $row_RecEventT['term_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                  <option value="1" <?php if (!(strcmp(1, $row_RecEventT['term_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                </select>
              </label></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  mysql_select_db($database_connect2data, $connect2data);  
  $query_RecCheckMainNews = "SELECT * FROM data_set AS D WHERE D.d_class1='mainNews' AND D.d_class2='eventT' AND D.d_class3='$term_id'";  
  ...
  if($totalRows_RecCheckMainNews==0){
    ...
    $updateSQL = sprintf("UPDATE data_set SET d_title=%s WHERE d_class3=%s AND d_class1='mainNews' AND d_class2='eventT'",
       GetSQLValueString($name, "text"),
       GetSQLValueString($term_id, "int"));
    ...
    $selectSQL = sprintf("SELECT d_id FROM data_set WHERE d_class3=%s AND d_class1='mainNews' AND d_class2='eventT'",       
       GetSQLValueString($term_id, "int"));
