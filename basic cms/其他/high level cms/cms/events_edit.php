...
$query_RecEvents = sprintf("SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_id = %s", GetSQLValueString($colname_RecEvents, "int"));
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecEventsC = "SELECT * FROM class_set WHERE c_parent = 'eventsC' AND c_active='1' ORDER BY c_sort ASC, c_id DESC";
...
if (isset($_SESSION['selected_Years'])){
	$G_selected1 = $_SESSION['selected_Years'] = $row_RecEvents['d_class2'];
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改活動行程</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
...
do {  
?>
            	      <option value="<?php echo $row_RecYears['c_id']?>" <?php if (!(strcmp($row_RecYears['c_id'], $row_RecEvents['d_class2']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecYears['c_title']?></option>
            	      <?php
...
?>
                ...
                <tr>
                  <td align="center" bgcolor="#e5ecf6" class="table_col_title">活動分類</td>
                  <td><label>
                    <select name="d_class5" id="d_class5" class="table_data">
                      <?php
                      do {  
                      ?>
                      <option value="<?php echo $row_RecEventsC['c_id']?>" <?php if (!(strcmp($row_RecEventsC['c_id'], $row_RecEvents['d_class5']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecEventsC['c_title']?></option>
                      <?php
                      } while ($row_RecEventsC = mysql_fetch_assoc($RecEventsC));
                        $rows = mysql_num_rows($RecEventsC);
                        if($rows > 0) {
                            mysql_data_seek($RecEventsC, 0);
                          $row_RecEventsC = mysql_fetch_assoc($RecEventsC);
                        }
                      ?>
                    </select>
                ...
                <td class="table_data">
                  ...
                  <div class="relationshipRadio">
                    <label for="relationship_1">
                      <input name="relationship" type="radio" id="relationship_1" value="0" <?php if (!(strcmp(0, $row_RecEvents['d_class4'])) || is_null($row_RecEvents['d_class4'])) {echo ' checked="checked"';} ?> />
                          無
                      </label>
                    <label for="relationship_2">
                      <input name="relationship" type="radio" id="relationship_2" value="1" <?php if (!(strcmp(1, $row_RecEvents['d_class4']))) {echo ' checked="checked"';} ?>/>
                          關聯
                      </label>
                  </div>
                  <div class="relationshipSelectGroup hide">
                    ...
                    <select name="d_class3" id="d_class3">
                      ...
                      do {  
                      ?>
                          <option value="<?php echo $row_RecArtistsT['term_id']?>"<?php if (!(strcmp($row_RecArtistsT['term_id'], $row_RecEvents['d_class3']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecArtistsT['name'].' '.$row_RecArtistsT['name_en']?></option>
                          <?php
            	...
            	<tr>
            	  ...
            	  <td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecEvents['d_title']; ?>" size="80" />
            	    </td>
            	  <td bgcolor="#e5ecf6">                  
            	...
            	<tr>
            	  ...
            	  <td class="table_data"><textarea name="d_content" cols="100" rows="20" class="table_data " id="d_content"><?php echo $row_RecEvents['d_content']; ?></textarea></td>
                ...
                <tr>
                  ...
                  <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo $row_RecEvents['d_date']; ?>" size="50" /></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
            	<tr>
            	  ...
            	  <td><label>
            	    <select name="d_active" class="table_data" id="d_active">
            	      <option value="0" <?php if (!(strcmp(0, $row_RecEvents['d_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
            	      <option value="1" <?php if (!(strcmp(1, $row_RecEvents['d_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
            	      </select>
            	    </label></td>
            	  <td bgcolor="#e5ecf6">&nbsp;</td>
          	  </tr>
    ...
    <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecEvents['d_id']; ?>" />
    <input name="d_sort" type="hidden" id="d_sort" value="<?php echo $row_RecEvents['d_sort']; ?>" />
    </form>
...
if( is_null($d_class4) ){
 ...
 //$y = substr($d_date, 0, 4);
  ...
  $updateSQL = sprintf("UPDATE data_set SET d_title=%s, d_content=%s, d_class2=%s, d_class3=%s, d_class4=%s, d_class5=%s, d_date=%s, d_active=%s WHERE d_id=%s",
  ...
  //$updateGoTo = "events_list.php?selected1=".$class2."&selected2=".$class3."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$_POST['d_id']."&totalRows_RecEvents=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  $updateGoTo = "events_list.php?selected1=".$d_class2."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$d_id."&totalRows=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
