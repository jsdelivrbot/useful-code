...
$query_RecNews = sprintf("SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_id = %s", GetSQLValueString($colname_RecNews, "int"));
...
if (isset($row_RecNews['d_id'])) {
  $colname_RecImageCover = $row_RecNews['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
...
if (isset($_SESSION['selected_NewsC'])){
	$G_sel = $_SESSION['selected_NewsC'] = $row_RecNews['d_class2'];
	//echo 'G_sel = '.$G_sel;
}*/
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改最新消息</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <div class="groupBtn">
      ...
      if( (isset($row_RecNews['d_id'])&&$row_RecNews['d_id']!='') && (isset($row_RecNews['d_class2'])&&$row_RecNews['d_class2']!='') ){
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."news_detail.php?sel=".$row_RecNews['d_class2']."&id=".$row_RecNews['d_id'];
      }else{
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."news.php";
      }
...
do {  
?>
            	      <option value="<?php echo $row_RecNewsC['c_id']?>" <?php if (!(strcmp($row_RecNewsC['c_id'], $row_RecNews['d_class2']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecNewsC['c_title']?></option>
            	      <?php
...
?>
            	...
            	<tr>
            	  ...
            	  <td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecNews['d_title']; ?>" size="80" />
            	    </td>
            	  <td bgcolor="#e5ecf6">                  
            	...
            	<tr>
            	  ...
            	  <td class="table_data"><textarea name="d_content" cols="100" rows="20" class="table_data tiny" id="d_content"><?php echo $row_RecNews['d_content']; ?></textarea></td>
              ...
              <tr>
                ...
                <td class="table_data">
                  ...
                  <div class="radioGroup">
                    <label for="d_price1_1">
                      <input name="d_price1" type="radio" id="d_price1_1" value="1" <?php if (!(strcmp(1, $row_RecNews['d_price1']))) {echo 'checked="CHECKED"';} ?>>
                      版型1
                    </label>
                  </div>
                  ...
                  <div class="radioGroup">
                    <label for="d_price1_2">
                      <input name="d_price1" type="radio" id="d_price1_2" value="2" <?php if (!(strcmp(2, $row_RecNews['d_price1']))) {echo 'checked="CHECKED"';} ?>>
                      版型2
                    </label>
                  </div>
                ...
                <tr>
                  ...
                  <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo $row_RecNews['d_date']; ?>" size="50" /></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
            	<tr>
            	  ...
            	  <td><label>
            	    <select name="d_active" class="table_data" id="d_active">
            	      <option value="0" <?php if (!(strcmp(0, $row_RecNews['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
            	      <option value="1" <?php if (!(strcmp(1, $row_RecNews['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option>
            	      </select>
            	    </label></td>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecNews['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option><option value="0" <?php if (!(strcmp(0, $row_RecNews['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
                  <tr>
                    <td align="center" bgcolor="#e5ecf6" class="table_col_title">目前內容圖片<a name="imageEdit" id="imageEdit"></a></td>
                    <td><?php do { ?>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 //$y = substr($d_date, 0, 4);
  ...
  $updateSQL = sprintf("UPDATE data_set SET d_title=%s, d_content=%s, d_class2=%s, d_price1=%s, d_date=%s, d_active=%s, d_pub=%s WHERE d_id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
	...
	$image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "news","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
    ...
    if(isset($_FILES['imageCover'])){
      ...
      $image_result = bannerImage_process($_FILES['imageCover'], $_REQUEST['imageCover_title'], "newsCover","add", $imagesSize[$_SESSION['nowMenu'].'Cover']['IW'], $imagesSize[$_SESSION['nowMenu'].'Cover']['IH']);
        //echo count($image_result);
        //echo $image_result[0][0];
  ...
  //$updateGoTo = "news_list.php?selected1=".$class2."&selected2=".$class3."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$_POST['d_id']."&totalRows_RecNews=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];

  $stringLink = "?sel=".$G_sel."&now_d_id=".$d_id."&totalRows_RecOwner=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  
  $updateGoTo = "news_list.php".$stringLink;
