...
function changeSort(pageNum, totalRows, now_d_id, change_num, selected1) { //v1.0
 ...
 window.location.href="news_list.php?selected1="+<?php echo $G_selected1;?>+"&changeSort=1"+"&now_d_id="+now_d_id+"&change_num="+change_num+"&pageNum="+pageNum+"&totalRows="+totalRows;
}
//-->
</script>
<!-- InstanceEndEditable -->
	...
	<form action="news_process.php" method="post" name="form1" id="form1">
      <?php if ($totalRows > 0) { // Show if recordset not empty ?>
		...
		echo "<tr bgcolor='#E4E4E4'>";}
          ...
          <?php
		mysql_select_db($database_connect2data, $connect2data);
		$query_RecImage = "SELECT * FROM file_set  WHERE file_type='imageCover' AND file_d_id = ".$row_RecNews['d_id'];
  ...
  <?php
	...
	<?php
          ...
          <td align="center"  class="table_data">
            ...
            <?php //發佈狀態
            if(in_array(5,$_SESSION['MM_Limit']['a5'])){
		        if($row_RecNews['d_pub']){
		          echo "<a href='".$row_RecNews['d_pub']."' rel='".$row_RecNews['d_id']."' class='pubD' title='發佈'>發佈</a>";
		        }else{
		          echo "<a href='".$row_RecNews['d_pub']."' rel='".$row_RecNews['d_id']."' class='pubD' title='草稿'>草稿</a>";
		        }
		      }else{
		        if($row_RecNews['d_pub']){
		          echo "發佈";
