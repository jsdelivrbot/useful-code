...
$colname_RecOrigin = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecOrigin = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecOrigin = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_RecOrigin, "int"));
$RecOrigin = mysql_query($query_RecOrigin, $connect2data) or die(mysql_error());
$row_RecOrigin = mysql_fetch_assoc($RecOrigin);
$totalRows_RecOrigin = mysql_num_rows($RecOrigin);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改唐獎源起</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">標題</td>
          	    	<td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecOrigin['d_title']; ?>" size="70" />
          	    	  <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecOrigin['d_id']; ?>" /></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	    	</tr>
            	<tr>
            	  <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容</td>
            	  <td><textarea name="d_content" cols="100" rows="20" class="table_data tiny" id="d_content"><?php echo $row_RecOrigin['d_content']; ?></textarea></td>
                ...
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
          	    	<td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_RecOrigin['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_RecOrigin['d_date']))) ) ? date("Y-m-d H:i:s") : $row_RecOrigin['d_date']; ?>" size="50"></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">在網頁顯示</td>
          	        ...
          	        <select name="d_active" class="table_data" id="d_active">
          	          <option value="1" <?php if (!(strcmp(1, $row_RecOrigin['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_RecOrigin['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
       	            </select></label></td>
          	     	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateGoTo = "origin_list.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING']."&pageNum_RecOrigin=".$_SESSION["ToPage"];
  }
  ...
  }else
  {
    ...
    echo "$('.pubBtn').attr('href', '../origin.php');";
    echo "$('.finishBtn').show();";
    echo "$('.pubBtn').show();";
    echo "});";
    echo "</script>";*/
