...
if (isset($_SESSION['selected_articleSubT'])){
  $G_sel = $_SESSION['selected_articleSubT'];
}
            	...
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">子分類名稱</td>
          	    	<td width="532"><input name="name" type="text" class="table_data" id="name" size="50">
          	    	  <input name="c_parent" type="hidden" id="c_parent" value="articleSubT" /></td>
...
<?php
  ...
  //----------插入圖片資料到資料庫begin(須放入插入主資料後)----------
	...
	$insertSQL = sprintf("INSERT INTO term_taxonomy (term_id, description, taxonomy, parent) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($new_data_num, "int"),
  ...
  /*$insertSQL = sprintf("INSERT INTO term_relationships (object_id, term_taxonomy_id, term_order) VALUES (%s, %s, 1)",
                       GetSQLValueString($new_data_num, "int"),
  ...
  mysql_select_db($database_connect2data, $connect2data);
  $query_RecArticleSubT = "SELECT * FROM terms AS T NATURAL JOIN term_taxonomy AS TT WHERE TT.taxonomy='articleSubT' AND TT.parent=$parent ORDER BY term_sort ASC, term_id DESC";
  ...
  //$query_RecArticleSubT = "SELECT * FROM terms AS T NATURAL JOIN term_taxonomy AS TT WHERE TT.taxonomy='articleSubT' AND TT.parent=5 ORDER BY term_sort ASC, term_id DESC";

  //$query_RecArticle = "SELECT * FROM term_relationships AS TR, data_set AS D, terms AS T WHERE TR.term_taxonomy_id = T.term_id AND D.d_id = TR.object_id AND T.term_id='$tagTMP' ORDER BY term_order ASC, d_date DESC";

  $_SESSION['totalRows'] = $totalRows_RecArticleSubT;

  $_SESSION['original_selected'] = $_SESSION['selected_articleSubT'] = $parent;
	
   
  $insertGoTo = "articleSubT_list.php?sel=".$parent."&pageNum=0&totalRows_RecArticleSubT=".$_SESSION['totalRows']."&changeSort=1&now_term_id=".$new_data_num."&change_num=1";
  //$stringLink = "?sel=".$tagTMP."&pageNum=0&totalRows_RecArticle=".$_SESSION['totalRows']."&changeSort=1&now_d_id=".$new_data_num."&change_num=1";
  if (isset($_SERVER['QUERY_STRING'])) {
