...
if(!in_array(5,$_SESSION['MM_Limit']['a3'])){
	header("Location: articleSubT_list.php");
}
...
$colname_RecArticleSubT = "-1";
if (isset($_GET['term_id'])) {
  $colname_RecArticleSubT = $_GET['term_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecArticleSubT = sprintf("SELECT * FROM terms AS T NATURAL JOIN term_taxonomy AS TT WHERE T.term_id = %s AND TT.taxonomy='articleSubT'", GetSQLValueString($colname_RecArticleSubT, "int"));
...
$G_sel = '';
if (isset($_SESSION['selected_articleSubT'])){
  $G_sel = $_SESSION['selected_articleSubT'] = $row_RecArticleSubT['parent'];
  //echo 'G_sel = '.$G_sel;
}
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="3" cellpadding="5">
              <tr>
                ...
                <td>
                  <label>
                    <select data-placeholder="請選擇文章分類..." class="chosen-select table_data" tabindex="4" name="parent" id="parent">
                      <?php
                      do {
                        $selA = explode(',',$row_RecArticleSubT['parent']);
            ...
            <tr>
              <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">子分類名稱</td>
              <td width="516"><input name="name" type="text" class="table_data" id="name" value="<?php echo $row_RecArticleSubT['name']; ?>" size="50" />
                  <input name="term_id" type="hidden" id="term_id" value="<?php echo $row_RecArticleSubT['term_id']; ?>" /></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>

            <tr>
              <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">子分類英文名稱</td>
              <td width="516"><input name="name_en" type="text" class="table_data" id="name_en" value="<?php echo $row_RecArticleSubT['name_en']; ?>" size="50" />
                  </td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
            ...
            <tr>
              ...
              <td width="516"><label>
                <select name="term_active" class="table_data" id="term_active">
                  <option value="0" <?php if (!(strcmp(0, $row_RecArticleSubT['term_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                  <option value="1" <?php if (!(strcmp(1, $row_RecArticleSubT['term_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                </select>
              </label></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateSQL = sprintf("UPDATE term_taxonomy SET description=%s, parent=%s WHERE term_id=%s",
  ...
  /*******************************************************************/
    ...
    mysql_select_db($database_connect2data, $connect2data);  
    $query_RecCheckMainNews = "SELECT * FROM data_set AS D WHERE D.d_class1='mainNews' AND D.d_class2='articleSubT' AND D.d_class3='$term_id'";  
    ...
    if($totalRows_RecCheckMainNews==0){
      $insertSQL = sprintf("INSERT INTO data_set (d_title, d_class1, d_class2, d_class3, d_active, d_pub, d_sort) VALUES (%s, %s, %s, %s, 1, 0, 1)",
                         ...
                         GetSQLValueString('articleSubT', "text"),
                         GetSQLValueString($term_id, "int"));
      ...
      $updateSQL = sprintf("UPDATE data_set SET d_title=%s WHERE d_class3=%s AND d_class1='mainNews' AND d_class2='articleSubT'",
         GetSQLValueString($name, "text"),
         GetSQLValueString($term_id, "int"));
      ...
      $selectSQL = sprintf("SELECT d_id FROM data_set WHERE d_class3=%s AND d_class1='mainNews' AND d_class2='articleSubT'",       
         GetSQLValueString($term_id, "int"));
  ...
  $_SESSION['original_selected'] = $_SESSION['selected_articleSubT'] = $parent;
  
  $updateGoTo = "articleSubT_list.php?sel=".$G_sel."&now_term_id=".$term_id."&totalRows_RecArticleSubT=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  if (isset($_SERVER['QUERY_STRING'])) {
