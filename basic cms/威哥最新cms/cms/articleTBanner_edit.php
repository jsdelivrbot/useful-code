...
if(!in_array(5,$_SESSION['MM_Limit']['a3'])){
	header("Location: articleTBanner_list.php");
}
...
$colname_RecArticleTBanner = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecArticleTBanner = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecArticleTBanner = sprintf("SELECT * FROM term_relationships AS TR, data_set AS D, terms AS T WHERE TR.term_taxonomy_id = T.term_id AND D.d_id = TR.object_id AND D.d_class1='articleTBanner' AND D.d_id = %s", GetSQLValueString($colname_RecArticleTBanner, "int"));
...
$totalRows_RecArticleTBanner = mysql_num_rows($RecArticleTBanner);

if($totalRows_RecArticleTBanner==0){
  header("Location: articleTBanner_list.php");
}
...
if (isset($_SESSION['selected_articleT'])){
	$G_sel = $_SESSION['selected_articleT'] = $row_RecArticleTBanner['term_id'];
	//echo 'G_sel = '.$G_sel;
}
...
$_SESSION['nowMenu']= "articleTBanner";
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改分類列表頁banner</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <div class="groupBtn">
      ...
      if( (isset($row_RecArticleTBanner['d_id'])&&$row_RecArticleTBanner['d_id']!='') && (isset($G_sel)&&$G_sel!='') ){

        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."articleTBanner_detail.php?cat=".$G_sel."&id=".$_GET['d_id'];

      }else{
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."articleTBanner.php";
      }
            	  ...
            	  <td>
            	    <label>
            	      <select data-placeholder="請選擇分類..." class="chosen-select table_data" tabindex="4" name="d_class2[]" id="d_class2">
            	        <?php
do {
	$selA = explode(',',$row_RecArticleTBanner['d_tag']);
...
?>
            	        <option value="<?php echo $row_RecArticleT['term_id']?>"<?php echo $sel; ?>><?php echo $row_RecArticleT['name']?><?php //echo $row_RecArticleT['term_id']?></option>
            	        <?php
...
?>
            	...
            	<tr>
            	  ...
            	  <td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecArticleTBanner['d_title']; ?>" size="50" />
          	    </td>
            	  <td bgcolor="#e5ecf6">                  
            	    <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecArticleTBanner['d_id']; ?>" />
                    <input name="term_order" type="hidden" id="term_order" value="<?php echo $row_RecArticleTBanner['term_order']; ?>" /><?php //echo $row_RecUnitary['term_order']; ?>
                  </td>
          	  </tr>

              <tr>
                <td align="center" bgcolor="#e5ecf6" class="table_col_title">英文標語</td>
                <td><input name="d_title_en" type="text" class="table_data" id="d_title_en" value="<?php echo $row_RecArticleTBanner['d_title_en']; ?>" size="50" />
                </td>
                ...
                <tr>
                  ...
                  <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo date("Y-m-d", strtotime($row_RecArticleTBanner['d_date'])); ?>" size="50" /></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_sale" class="table_data" id="d_sale">
                      <option value="0" <?php if (!(strcmp(0, $row_RecArticleTBanner['d_sale']))) {echo "selected=\"selected\"";} ?>>非進行中</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecArticleTBanner['d_sale']))) {echo "selected=\"selected\"";} ?>>進行中</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                <?php } ?>
                <tr>
                  ...
                  <td><label>
                    <select name="d_active" class="table_data" id="d_active">
                      <option value="0" <?php if (!(strcmp(0, $row_RecArticleTBanner['d_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecArticleTBanner['d_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                    </select>
                  </label></td>
                  <td bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecArticleTBanner['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option>
                      <option value="0" <?php if (!(strcmp(0, $row_RecArticleTBanner['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                <?php } ?>
 ...
 $d_data3	= checkV('d_data3');
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
	...
	if(isset($_FILES['image']) && $_FILES['image']!=''){
  	$image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "articleTBanner","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
  ...
  $stringLink = "?sel=".$G_sel."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$d_id."&totalRows_RecArticleTBanner=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  
  $updateGoTo = "articleTBanner_list.php".$stringLink;


  //$updateGoTo = "articleTBanner_list.php?sel=".$class2."&selected2=".$class3."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$_POST['d_id']."&totalRows_RecArticleTBanner=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  //$updateGoTo = "articleTBanner_list.php?sel=".$G_sel."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$d_id."&totalRows_RecArticleTBanner=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
