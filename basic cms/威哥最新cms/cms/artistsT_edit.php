...
<?php require_once('photo_process_artists.php'); ?>
<?php
...
if(!in_array(5,$_SESSION['MM_Limit']['a4'])){
	header("Location: artistsT_list.php");
}
...
$colname_RecArtistsT = "-1";
if (isset($_GET['term_id'])) {
  $colname_RecArtistsT = $_GET['term_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecArtistsT = sprintf("SELECT * FROM terms AS T LEFT JOIN term_taxonomy AS TT ON T.term_id=TT.term_id  WHERE T.term_id = %s", GetSQLValueString($colname_RecArtistsT, "int"));
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecImage = sprintf("SELECT * FROM file_set WHERE file_d_id = %s AND file_type = 'artistsCover' ORDER BY file_id DESC", GetSQLValueString($colname_RecImage, "int"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改Artists</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="3" cellpadding="5">
            <tr>
              ...
              <td width="516"><input name="name" type="text" class="table_data" id="name" value="<?php echo $row_RecArtistsT['name']; ?>" size="50" />
                  <input name="term_id" type="hidden" id="term_id" value="<?php echo $row_RecArtistsT['term_id']; ?>" /></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>

            <tr>
              <td align="center" bgcolor="#e5ecf6" class="table_col_title">英文名稱</td>
              <td><input name="name_en" type="text" class="table_data" id="name_en" value="<?php echo $row_RecArtistsT['name_en']; ?>" size="50" />
                  </td>
              ...
              <tr>
                  ...
                  <td>
                      <textarea name="description" cols="100" rows="20" class="table_data" id="description"><?php echo $row_RecArtistsT['description']; ?></textarea>
                    </td>
            ...
            <tr>
              ...
              <td width="516"><label>
                <select name="term_active" class="table_data" id="term_active">
                  <option value="0" <?php if (!(strcmp(0, $row_RecArtistsT['term_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                  <option value="1" <?php if (!(strcmp(1, $row_RecArtistsT['term_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                </select>
              </label></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "artists","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
    ...
    for($j=1;$j<count($image_result);$j++)
    {
        $insertSQL = sprintf("INSERT INTO file_set (file_name, file_link1, file_link2, file_link3, file_type, file_d_id, file_title, file_show_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString("artistsCover", "text"),
                       GetSQLValueString($_POST['term_id'], "int"),
