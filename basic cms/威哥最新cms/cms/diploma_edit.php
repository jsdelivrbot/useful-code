...
if(!in_array(5,$_SESSION['MM_Limit']['a4'])){
	header("Location: diploma_list.php");
}
...
$colname_RecDiploma = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecDiploma = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecDiploma = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_RecDiploma, "int"));
$RecDiploma = mysql_query($query_RecDiploma, $connect2data) or die(mysql_error());
$row_RecDiploma = mysql_fetch_assoc($RecDiploma);
$totalRows_RecDiploma = mysql_num_rows($RecDiploma);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改唐獎證書</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">標題</td>
          	    	<td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecDiploma['d_title']; ?>" size="70" />
          	    	  <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecDiploma['d_id']; ?>" /></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	    	</tr>

              <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">英文標題</td>
                  <td><input name="d_title_en" type="text" class="table_data" id="d_title_en" value="<?php echo $row_RecDiploma['d_title_en']; ?>" size="70" />
                    </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
            	...
            	<tr>
            	  ...
            	  <td><textarea name="d_content" cols="100" rows="20" class="table_data tiny" id="d_content"><?php echo $row_RecDiploma['d_content']; ?></textarea></td>
                ...
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
          	    	<td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_RecDiploma['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_RecDiploma['d_date']))) ) ? date("Y-m-d H:i:s") : $row_RecDiploma['d_date']; ?>" size="50"></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">在網頁顯示</td>
          	        ...
          	        <select name="d_active" class="table_data" id="d_active">
          	          <option value="1" <?php if (!(strcmp(1, $row_RecDiploma['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_RecDiploma['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
       	            </select></label></td>
          	     	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecDiploma['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option><option value="0" <?php if (!(strcmp(0, $row_RecDiploma['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 $d_id       = checkV('d_id');
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
		...
		//$image_result=image_process("diploma","add", "0", "0");
		$image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "diploma","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
	    ...
	    //----------插入檔案資料到資料庫begin(須放入插入主資料後)----------
			...
			$file_result=file_process("diploma","add");
  ...
  $updateGoTo = "diploma_list.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING']."&pageNum_RecDiploma=".$_SESSION["ToPage"];
  }
