...
<?php require_once('photo_process_event.php'); ?>
...
$colname_RecEvent = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecEvent = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecEvent = sprintf("SELECT * FROM term_relationships AS TR, data_set AS D, terms AS T WHERE TR.term_taxonomy_id = T.term_id AND D.d_id = TR.object_id AND D.d_class1='event' AND D.d_id = %s", GetSQLValueString($colname_RecEvent, "int"));
$RecEvent = mysql_query($query_RecEvent, $connect2data) or die(mysql_error());
$row_RecEvent = mysql_fetch_assoc($RecEvent);
$totalRows_RecEvent = mysql_num_rows($RecEvent);

if($totalRows_RecEvent==0){
  header("Location: event_list.php");
}
...
$G_sel = '';
if (isset($_SESSION['selected_eventT'])){
	$G_sel = $_SESSION['selected_eventT'] = $row_RecEvent['term_id'];
	//echo 'G_sel = '.$G_sel;
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改活動訊息</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <div class="groupBtn">
      ...
      if( (isset($row_RecEvent['d_id'])&&$row_RecEvent['d_id']!='') && (isset($G_sel)&&$G_sel!='') ){

        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."event_detail.php?cat=".$G_sel."&id=".$_GET['d_id'];

      }else{
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."event.php";
      }
            	  ...
            	  <td>
            	    <label>
            	      <select data-placeholder="請選擇分類..." class="chosen-select table_data" tabindex="4" name="d_class2[]" id="d_class2">
            	        <?php
do {
	$selA = explode(',',$row_RecEvent['d_tag']);
	if (in_array($row_RecEventT['term_id'], $selA)){
		$sel = "selected=\"selected\"";
	}else{
		$sel = "";
	}
?>
            	        <option value="<?php echo $row_RecEventT['term_id']?>"<?php echo $sel; ?>><?php echo $row_RecEventT['name']?><?php //echo $row_RecEventT['term_id']?></option>
            	        <?php
...
?>
            	...
            	<tr>
            	  ...
            	  <td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecEvent['d_title']; ?>" size="50" />
          	    </td>
            	  <td bgcolor="#e5ecf6">                  
            	    <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecEvent['d_id']; ?>" />
                    <input name="term_order" type="hidden" id="term_order" value="<?php echo $row_RecEvent['term_order']; ?>" /><?php //echo $row_RecUnitary['term_order']; ?>
                  </td>
          	  </tr>

            	<tr>
            	  <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容</td>
            	  <td><textarea name="d_content" cols="100" rows="20" class="table_data tiny" id="d_content"><?php echo $row_RecEvent['d_content']; ?></textarea></td>
                ...
                <tr>
                  ...
                  <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo date("Y-m-d", strtotime($row_RecEvent['d_date'])); ?>" size="50" /></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_sale" class="table_data" id="d_sale">
                      <option value="0" <?php if (!(strcmp(0, $row_RecEvent['d_sale']))) {echo "selected=\"selected\"";} ?>>非進行中</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecEvent['d_sale']))) {echo "selected=\"selected\"";} ?>>進行中</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                <tr>
                  ...
                  <td><label>
                    <select name="d_active" class="table_data" id="d_active">
                      <option value="0" <?php if (!(strcmp(0, $row_RecEvent['d_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecEvent['d_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                    </select>
                  </label></td>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecEvent['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option>
                      <option value="0" <?php if (!(strcmp(0, $row_RecEvent['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 $d_data3	= checkV('d_data3');
  ...
  $updateSQL = sprintf("UPDATE data_set SET d_title=%s, d_title_en=%s, d_content=%s, d_tag=%s, d_class2=%s, d_class3=%s, d_class4=%s, d_class5=%s, d_class6=%s, d_data1=%s, d_data2=%s, d_data3=%s, d_price1=%s, d_price2=%s, d_date=%s, d_active=%s, d_pub=%s, d_sale=%s WHERE d_id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
    ...
    mysql_select_db($database_connect2data, $connect2data);
    $query_RecEventT = "SELECT * FROM terms AS T NATURAL JOIN term_taxonomy AS TT WHERE TT.taxonomy='eventT' AND T.term_active='1' AND term_id='$tagO'";
    $RecEventT = mysql_query($query_RecEventT, $connect2data) or die(mysql_error());
    $row_RecEventT = mysql_fetch_assoc($RecEventT);
    $totalRows_RecEventT = mysql_num_rows($RecEventT);
	 
		//$d_tagTXT = $d_tagTXT.$row_RecEventT['name'].",".$row_RecEventT['name_en'].",";

    if($row_RecEventT['name']!=''){
      $d_tagTXT = $d_tagTXT.$row_RecEventT['name'].",";
    }
    if($row_RecEventT['name_en']!=''){
      $d_tagTXT = $d_tagTXT.$row_RecEventT['name_en'].",";
    }
	}
  ...
  $Result1 = mysql_query($updateSQL, $connect2data) or die(mysql_error());
	...
	$image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "event","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
  ...
  $_SESSION['original_selected'] = $_SESSION['selected_eventT'] = $tagTMP;
  $G_sel = $tagTMP;


  $stringLink = "?sel=".$G_sel."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$d_id."&totalRows_RecEvent=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  
  $updateGoTo = "event_list.php".$stringLink;


  //$updateGoTo = "event_list.php?sel=".$class2."&selected2=".$class3."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$_POST['d_id']."&totalRows_RecEvent=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
  //$updateGoTo = "event_list.php?sel=".$G_sel."&changeSort=1&change_num=".$_POST['term_order']."&now_d_id=".$d_id."&totalRows_RecEvent=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
