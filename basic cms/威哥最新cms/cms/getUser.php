...
$query_RecUser = sprintf("SELECT user_id, user_account, user_level, user_limit, user_active FROM `admin` WHERE user_account = %s AND user_status='active'", GetSQLValueString($colname_RecUser, "text"));
...
for($i=1; $i<=16; $i++){
	...
	if((isset($row_RecLevelAuthority['a_'.$i])) && ($row_RecLevelAuthority['a_'.$i]!=NULL)){
