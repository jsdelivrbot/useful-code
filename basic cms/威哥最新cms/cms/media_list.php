  ...
  <?php $_SESSION['totalRows']=$totalRows_RecMedia; ?></td>
    ...
    <?php
        ...
        <td align="center"  class="table_data">
            ...
            <?php //進行中
            if(in_array(5,$_SESSION['MM_Limit']['a7'])){
		        if($row_RecMedia['d_sale']){
		          echo "<a href='".$row_RecMedia['d_sale']."' rel='".$row_RecMedia['d_id']."' class='pubA' title='進行中'>進行中</a>";
		        }else{
		          echo "<a href='".$row_RecMedia['d_sale']."' rel='".$row_RecMedia['d_id']."' class='pubA' title='非進行中'>非進行中</a>";
		        }
		      }else{
		        if($row_RecMedia['d_sale']){
		          echo "進行中";
