...
//一般用
function image_process($FILES_A, $file_title, $file_name, $deal_type, $image_width, $image_height, $TypeImg){
	...
	for($j=0;$j<count($FILES_A['name']);$j++)
	{
		...
		if($tmp_name!='')//如果有上傳檔案
		{
			...
			if($image_type=="jpg" || $image_type=="gif" || $image_type=="bmp" || $image_type=="png" || $image_type=="tif")
			{
			            ...
			            $photo_name = md5($file_name.$TypeImg.$j.time());
						////echo $photo_name."<br>";
						...
						*/
						...
						$all_image_name[$j+1][4] = NULL;//檔案title(說明)
						/*if($deal_type=="add"){
							$all_image_name[$j+1][4]=$file_title[$j];//檔案title(說明)
						}*/
...
//photo用
function photo_process($FILES_A, $file_title, $file_content, $file_name, $deal_type, $image_width, $image_height){
	////echo count($FILES_A['name']);//上傳物件數量
	////echo count($_REQUEST[image_title]);//上傳物件的說明之數量
	//echo "imageH = ".$image_height."<br/>";
		$all_image_name = array();//建立回傳的資料陣列
		...
		if($tmp_name!='')//如果有上傳檔案
		{
			...
			if($image_type=="jpg" || $image_type=="gif" || $image_type=="bmp" || $image_type=="png" || $image_type=="tif")
			{
						...
						*/
						...
						$all_image_name[$j+1][2]=$this_path2;//小圖檔案位置後台list用//中圖檔案位置index用
						$all_image_name[$j+1][3]=$this_path3;//list-小圖
						$all_image_name[$j+1][6]=$this_path4;//細節list-小圖
						
						//$all_image_name[$j+1][4]=$this_path4;//小圖檔案位置list用
						$all_image_name[$j+1][5]=$format;//判斷橫式或直式圖
						//echo 'j = '.$j.'<br>';
						//echo 'file_title = '.$file_title[$j].'<br>';$file_content
						$all_image_name[$j+1][4]=$file_title[$j];//檔案title(說明)
						$all_image_name[$j+1][9]=$file_content[$j];//檔案說明
						/*if($deal_type=="add"){
							$all_image_name[$j+1][4]=$file_title[$j];//檔案title(說明)
						}*/
