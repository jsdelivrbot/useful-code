		...
		if($tmp_name!='')//如果有上傳檔案
		{
			...
			if($image_type=="jpg" || $image_type=="gif" || $image_type=="bmp" || $image_type=="png" || $image_type=="tif")
			{
						...
						*/
						...
						$destW = 240;
						$destH = 158;
						$this_path5=$image_path."/".$file_name."_".$photo_name."_".intval($destW).'.'.$image_type;
						$this_image_path5 = "../".$this_path5;
						copy($FILES_A['tmp_name'][$j] , $this_image_path5 );
						imagesResize_4($this_image_path5,$this_image_path5,$destW,$destH);

						//縮小圖artists detail--小圖
						$destW = 208;
						$destH = 137;
						$this_path6=$image_path."/".$file_name."_".$photo_name."_".intval($destW).'.'.$image_type;
						$this_image_path6 = "../".$this_path6;
						copy($FILES_A['tmp_name'][$j] , $this_image_path6 );
						imagesResize_4($this_image_path6,$this_image_path6,$destW,$destH);
						...
						$all_image_name[$j+1][6]=$this_path4;//小圖檔案位置list用
						$all_image_name[$j+1][7]=$this_path5;//小圖檔案位置list用
						$all_image_name[$j+1][8]=$this_path6;//小圖檔案位置list用
...
//橫式:W是否大於設定W，如果有則依設定W按比例縮小，縮小後比對H是否大於設定H，如果有則依設定H按比例縮小
//直式:H是否大於設定H，如果有則依設定H按比例縮小，縮小後比對W是否大於設定W，如果有則依設定W按比例縮小
function imagesResize_6($src,$dest,$destW,$destH) { 
		if (file_exists($src)  && isset($dest)) { 
			//取得檔案資訊
			$srcSize   = getimagesize($src); 
			/*
...
//設定尺寸按比例縮小，如超過則會裁切
function imagesResize_7($src,$dest,$destW,$destH) { 
		if (file_exists($src)  && isset($dest)) { 
			//取得檔案資訊
			$srcSize   = getimagesize($src); 
			$srcExtension = $srcSize[2];
