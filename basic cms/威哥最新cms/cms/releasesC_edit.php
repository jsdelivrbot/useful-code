...
if(!in_array(5,$_SESSION['MM_Limit']['a6'])){
	header("Location: releasesC_list.php");
}
...
$colname_RecReleasesC = "-1";
if (isset($_GET['c_id'])) {
  $colname_RecReleasesC = $_GET['c_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecReleasesC = sprintf("SELECT * FROM class_set WHERE c_id = %s", GetSQLValueString($colname_RecReleasesC, "int"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改專輯分類</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="3" cellpadding="5">
            <tr>
              ...
              <td width="516"><input name="c_title" type="text" class="table_data" id="c_title" value="<?php echo $row_RecReleasesC['c_title']; ?>" size="50" />
                  <input name="c_id" type="hidden" id="c_id" value="<?php echo $row_RecReleasesC['c_id']; ?>" /></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
            <tr>
              <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">英文分類名稱</td>
              <td width="516"><input name="c_title_en" type="text" class="table_data" id="c_title_en" value="<?php echo $row_RecReleasesC['c_title_en']; ?>" size="50" />
                  </td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
            <tr>
              ...
              <td width="516"><label>
                <select name="c_active" class="table_data" id="c_active">
                  <option value="0" <?php if (!(strcmp(0, $row_RecReleasesC['c_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                  <option value="1" <?php if (!(strcmp(1, $row_RecReleasesC['c_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                </select>
              </label></td>
              <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
            </tr>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE class_set SET c_title=%s, c_title_en=%s, c_class=%s, c_content=%s, c_link=%s, c_active=%s WHERE c_id=%s",
