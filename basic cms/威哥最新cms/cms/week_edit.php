...
$colname_RecWeek = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecWeek = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecWeek = sprintf("SELECT * FROM data_set WHERE d_id = %s AND d_class1='week'", GetSQLValueString($colname_RecWeek, "int"));
$RecWeek = mysql_query($query_RecWeek, $connect2data) or die(mysql_error());
$row_RecWeek = mysql_fetch_assoc($RecWeek);
$totalRows_RecWeek = mysql_num_rows($RecWeek);
...
mysql_select_db($database_connect2data, $connect2data);
$query_RecTabs = sprintf("SELECT * FROM tab_set WHERE tab_d_id = %s AND tab_type = 'week' ORDER BY tab_sort ASC, tab_id ASC", GetSQLValueString($colname_RecTabs, "int"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改唐獎週</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <div class="groupBtn">
      ...
      if( (isset($row_RecWeek['d_id'])&&$row_RecWeek['d_id']!='') && (isset($row_RecWeek['d_date'])&&$row_RecWeek['d_date']!='') ){
        $reviewLink = "http://".$_SERVER['HTTP_HOST'].$nowFile."/"."week.php?cat=".$row_RecWeek['d_id']."&y=".(date("Y", strtotime($row_RecWeek['d_date'])));
      }else{
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">名稱</td>
          	    	<td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecWeek['d_title']; ?>" size="70" />
          	    	  <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecWeek['d_id']; ?>" /></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	    	</tr>

              <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">英文名稱</td>
                  <td><input name="d_title_en" type="text" class="table_data" id="d_title_en" value="<?php echo $row_RecWeek['d_title_en']; ?>" size="70" />
                    </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
              </table>
              ...
              ?>
                ...
                <tr>
                  ...
                  <td><input name="tab_title[]" type="text" class="table_data" id="tab_title<?php echo $i; ?>" value="<?php echo $row_RecTabs['tab_title']; ?>" size="70" />
                    </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">頁籤內容</td>
                  <td><textarea name="tab_content[]" cols="100" rows="20" class="table_data tiny" id="tab_content<?php echo $i; ?>"><?php echo $row_RecTabs['tab_content']; ?></textarea></td>
              ...
              <table width="100%" border="0" cellspacing="3" cellpadding="5">
                ...
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
          	    	<td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( (date("Y-m-d", strtotime($row_RecWeek['d_date']))=='') || (!(strcmp("0000-00-00 ", date("Y-m-d", strtotime($row_RecWeek['d_date']))))) ) ? date("Y-m-d H:i:s") : date("Y-m-d", strtotime($row_RecWeek['d_date'])); ?>" size="50"></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_sale" class="table_data" id="d_sale">
                      <option value="0" <?php if (!(strcmp(0, $row_RecWeek['d_sale']))) {echo "selected=\"selected\"";} ?>>非進行中</option>
                      <option value="1" <?php if (!(strcmp(1, $row_RecWeek['d_sale']))) {echo "selected=\"selected\"";} ?>>進行中</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">在網頁顯示</td>
          	        ...
          	        <select name="d_active" class="table_data" id="d_active">
          	          <option value="1" <?php if (!(strcmp(1, $row_RecWeek['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option>
                      <option value="0" <?php if (!(strcmp(0, $row_RecWeek['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
       	            </select></label></td>
          	     	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <tr>
                  ...
                  <td><label>
                    <select name="d_pub" class="table_data" id="d_pub">
                      <option value="1" <?php if (!(strcmp(1, $row_RecWeek['d_pub']))) {echo "selected=\"selected\"";} ?>>發佈</option><option value="0" <?php if (!(strcmp(0, $row_RecWeek['d_pub']))) {echo "selected=\"selected\"";} ?>>草稿</option>
                    </select></label></td>
                  <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                </tr>
                ...
                <?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
                  <tr>
                    ...
                    <td><?php do { ?>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td width="100" rowspan="2" align="center"><a href="../<?php echo $row_RecImage['file_link1']; ?>" class="fancyboxImg" rel="group" title="<?php echo $row_RecImage['file_title']; ?>"><img src="../<?php echo $row_RecImage['file_link2']; ?>" alt="" class="image_frame" width="100" /></a></td>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 $d_id  = checkV('d_id');
  ...
  $updateSQL = sprintf("UPDATE data_set SET d_title=%s, d_title_en=%s, d_content=%s, d_class2=%s,d_date=%s, d_active=%s, d_pub=%s, d_sale=%s WHERE d_id=%s",
  ...
  if(isset($_POST['tab_title'])){ //如果有資料
    ...
    $deleteSQL = sprintf("DELETE FROM tab_set WHERE tab_type='week' AND tab_d_id=%s",
  ...
  $updateGoTo = "week_list.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING']."&pageNum_RecWeek=".$_SESSION["ToPage"];
  }
