...
if(!in_array(5,$_SESSION['MM_Limit']['a10'])){
	header("Location: weneedu_list.php");
}
...
$colname_RecWeneedu = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecWeneedu = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecWeneedu = sprintf("SELECT * FROM data_set WHERE d_id = %s", GetSQLValueString($colname_RecWeneedu, "int"));
$RecWeneedu = mysql_query($query_RecWeneedu, $connect2data) or die(mysql_error());
$row_RecWeneedu = mysql_fetch_assoc($RecWeneedu);
$totalRows_RecWeneedu = mysql_num_rows($RecWeneedu);

if($totalRows_RecWeneedu==0){
  header("Location: weneedu_list.php");
}

$colname_RecWeneeduSection = "-1";
if (isset($_GET['d_id'])) {
  $colname_RecWeneeduSection = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_RecWeneeduSection = sprintf("SELECT * FROM tab_set WHERE tab_d_id = %s AND tab_type = 'weneeduSection' ORDER BY tab_sort ASC, tab_id ASC", GetSQLValueString($colname_RecWeneeduSection, "int"));
$RecWeneeduSection = mysql_query($query_RecWeneeduSection, $connect2data) or die(mysql_error());
$row_RecWeneeduSection = mysql_fetch_assoc($RecWeneeduSection);
$totalRows_RecWeneeduSection = mysql_num_rows($RecWeneeduSection);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td width="30%" class="list_title">修改誠徵作家</td>
            <td width="70%">&nbsp;</td>
        </tr>
	</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
  		<tr>
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5">
            	<tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">標題</td>
          	    	<td><input name="d_title" type="text" class="table_data" id="d_title" value="<?php echo $row_RecWeneedu['d_title']; ?>" size="70" />
          	    	  <input name="d_id" type="hidden" id="d_id" value="<?php echo $row_RecWeneedu['d_id']; ?>" /></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	    	</tr>
              <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">簡介</td>
                  <td>
                  <textarea name="d_content" cols="100" rows="10" class="table_data" id="d_content"><?php echo $row_RecWeneedu['d_content']; ?></textarea>
                  </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
              <tr>
                  ...
                  <td>
                  <textarea name="d_data1" cols="100" rows="5" class="table_data" id="d_data1"><?php echo $row_RecWeneedu['d_data1']; ?></textarea>
                  </td>
                <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
              </tr>
            </table>
            ...
            <table width="100%" border="0" cellspacing="3" cellpadding="5" id="addArea">
                ...
                <?php
                  if($totalRows_RecWeneeduSection>0){ 
                    $i=1;
                    do{
                ?>
                <tr>
                  <td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">章節中文標題<?php echo $i; ?></td>
                  <td><input name="tab_title[]" type="text" class="table_data" id="tab_title<?php echo $i; ?>" value="<?php echo $row_RecWeneeduSection['tab_title']; ?>" size="70" /></td>
                ...
                <tr>
                  ...
                  <td><input name="tab_content[]" type="text" class="table_data" id="tab_content<?php echo $i; ?>" value="<?php echo $row_RecWeneeduSection['tab_content']; ?>" size="70" /></td>
                ...
                <tr>
                  ...
                  <td>
                  <textarea name="tab_data1[]" cols="100" rows="10" class="table_data " id="tab_data1_<?php echo $i; ?>"><?php echo $row_RecWeneeduSection['tab_data1']; ?></textarea>
                  </td>
                <td width="250" bgcolor="#e5ecf6"><p class="red_letter">*請直接填寫章節內容。</p></td>
                </tr>
                <?php 
                    $i++;
                    } while ($row_RecWeneeduSection = mysql_fetch_assoc($RecWeneeduSection));
                  }
                ?>
              ...
              <table width="100%" border="0" cellspacing="3" cellpadding="5">
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">日期</td>
          	    	<td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo ( ($row_RecWeneedu['d_date']=='') || (!(strcmp("0000-00-00 00:00:00", $row_RecWeneedu['d_date']))) ) ? date("Y-m-d H:i:s") : $row_RecWeneedu['d_date']; ?>" size="50"></td>
        	    	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                <tr>
                	<td width="200" align="center" bgcolor="#e5ecf6" class="table_col_title">在網頁顯示</td>
          	        ...
          	        <select name="d_active" class="table_data" id="d_active">
          	          <option value="1" <?php if (!(strcmp(1, $row_RecWeneedu['d_active']))) {echo "selected=\"selected\"";} ?>>顯示</option><option value="0" <?php if (!(strcmp(0, $row_RecWeneedu['d_active']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
       	            </select></label></td>
          	     	<td width="250" bgcolor="#e5ecf6">&nbsp;</td>
      	      	</tr>
                ...
                <?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
                  <tr>
                    <td align="center" bgcolor="#e5ecf6" class="table_col_title">目前電腦版圖片</td>
                    <td><?php do { ?>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
      		    ...
      		    <tr>
              		<td align="center" bgcolor="#e5ecf6" class="table_col_title"><p>上傳電腦版圖片</p>              	    </td>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  if(isset($_POST['tab_title']) && isset($_POST['tab_data1'])){ //如果有資料
    ...
    $deleteSQL = sprintf("DELETE FROM tab_set WHERE tab_type='weneeduSection' AND tab_d_id=%s",
               GetSQLValueString($d_id, "int"));
    ...
    for($j=0; $j<count($tab_title); $j++){
      ...
      if($tab_title[$j]!='' && $tab_data1[$j]!=''){
        ...
        $insertSQL = sprintf("INSERT INTO tab_set (tab_d_id, tab_type, tab_title, tab_content, tab_data1, tab_sort) VALUES (%s, %s, %s, %s, %s, %s)",
                           GetSQLValueString($d_id, "int"),
                           GetSQLValueString('weneeduSection', "text"),
  ...
  //----------插入動態頁籤資料到資料庫end----------
      ...
      $image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "weneedu","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
    ...
    if(isset($_FILES['imageMobile']) && $_FILES['imageMobile']!=''){
      ...
      $image_result=image_process($_FILES['imageMobile'], $_REQUEST['imageMobile_title'], "weneedu"."Mobile","add", $imagesSize[$_SESSION['nowMenu']."Mobile"]['IW'], $imagesSize[$_SESSION['nowMenu']."Mobile"]['IH']);
      //echo count($image_result);
      //echo $image_result[0][0];
    ...
    if(0){
	    //----------插入檔案資料到資料庫begin(須放入插入主資料後)----------
			...
			$file_result=file_process("weneedu","add");
  ...
  $updateGoTo = "weneedu_list.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING']."&pageNum_RecWeneedu=".$_SESSION["ToPage"];
  }
