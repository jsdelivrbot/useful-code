...
if ($cart->itemCount == 0){
  ...
  if ($totalRows_RecMember == 0 && ( isset($_POST['addMember']) && $_POST['addMember']=='1') ) {
...
$insertSQL = sprintf("INSERT INTO order_set (client, c_gender, phone, cellphone, email, address, zipcode, r_client, r_gender, r_phone, r_cellphone, r_email, r_zipcode, r_address, delivery, transport, payment, invoice, insn, inname, in_zipcode, inadd, in_phone, notation, tfee, SubTotalAll, GrandTotal, m_id, m_account, datetime, RID) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
 GetSQLValueString($_POST['client'], "text"),
 ...
 GetSQLValueString($_POST['r_client'], "text"),
 GetSQLValueString($_POST['r_gender'], "int"),
 GetSQLValueString($_POST['r_phone'], "text"),
 GetSQLValueString($_POST['r_cellphone'], "text"),
 GetSQLValueString($_POST['r_email'], "text"),
 GetSQLValueString($r_zipcode, "int"),
 ...
 GetSQLValueString($_POST['in_zipcode'], "int"),
 GetSQLValueString($in_addressAll, "text"),
 GetSQLValueString($_POST['in_phone'], "text"),
...
<body>
  ...
  <form action="<?php echo $updateAction; ?>" method="post" enctype="multipart/form-data" name="formular" id="formular" class="formular">
    ...
    <ul class="order-list">
     ...
     <p class="pTitle"><?php echo urldecode($item['name']);?></p>
...
<div class="areacontent h-mb">
  <p>購物滿 <?php echo ($row_Freight['d_price1']!='')?$row_Freight['d_price1']:'499';?> 元(含)以上免運費，未滿 <?php echo ($row_Freight['d_price1']!='')?$row_Freight['d_price1']:'499';?> 元，運費為 <?php echo ($row_Freight['d_price2']!='')?$row_Freight['d_price2']:'100';?> 元。</p>
</div>
...
<form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="buyFormular" id="buyFormular" >
 ...
 <div class="datahere h-bb">
  <div>
   ...
   <label for="payment1">ATM 帳戶匯款</label>
   <input name="payment" type="radio" id="payment2" value="2" <?php if ($totalRows_RecMember == 0) {echo 'checked="CHECKED"';}?>  class="pay-radio">
   <label for="payment2">貨到付款</label>
			...
			<label for="r8">貨到付款</label>-->
		</div>

    <p id="payTxt1" class="unicorn">當您的訂單資料成立後，您需於 3 天內完成匯款，以利如期出貨。<br /><br />
    ...
    <p id="payTxt2" class="unicorn">貨到付款金額超過 2 萬元者，請改用ATM匯款，謝謝您。</p>

    <!--<p class="unicorn">貨到付款金額超過 2 萬元者，請改用ATM匯款，謝謝您。</p>-->
  </div>
