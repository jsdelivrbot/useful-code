...
if (isset($_POST['user_name'])) {
  $loginUsername=clear_str($_POST['user_name']);
  ...
  $LoginRS__query=sprintf("SELECT user_id, user_name, user_password, user_level, user_limit, user_active FROM `admin` WHERE user_name=%s AND user_password=%s AND user_active='1'",
   ...
   $insertSQL = sprintf("INSERT INTO admin_log (user_id, user_name, user_limit, user_active, login_date, login_ip, HTTP_USER_AGENT) VALUES (%s, %s, %s, %s, NOW(), %s, %s)",
   ...
   if ($loginFoundUser) {//帳密OK
      ...
      $LoginRS__query=sprintf("SELECT user_id, user_name, user_level, user_limit, user_active FROM `admin` WHERE user_name=%s AND user_password=%s AND user_active='1' AND user_status='active' AND user_loginerr<5",
         GetSQLValueString($loginUsername, "text"), GetSQLValueString($password, "text"));
      $LoginRS = mysql_query($LoginRS__query, $connect2data) or die(mysql_error());
      $row_LoginRS = mysql_fetch_assoc($LoginRS);
      $loginFoundUser = mysql_num_rows($LoginRS);
      ...
      $checkErrSql = sprintf("SELECT user_loginerr FROM `admin` WHERE user_name=%s",
                           GetSQLValueString($loginUsername, "text")); 
      mysql_select_db($database_connect2data, $connect2data);
      ...
      if($CheckErrRSTotal>0){ //表示有該帳號
         ...
         if($err>5){
            //更新被鎖
            $updateSQL = sprintf("UPDATE admin SET user_status=%s WHERE user_name=%s",
            ...
            //紀錄登入失敗次數
            $updateSQL = sprintf("UPDATE admin SET user_loginerr=%s WHERE user_name=%s",
...
<form action="<?php echo $loginFormAction; ?>" method="POST" name="form1" id="form1">
    ...
    <div class="art-logo-name"><img src="../img/logo-b.png"></div>
    <h2 class="art-logo-text">C+H</h2>
    <h3 class="login-cms-text">後端內容管理系統</h3>
    
    <div id="login-content">
    <ul id="login-input">
    	<li><span class="login-input-text"><label for="user_name">ACCOUNT</label></span><input type="text" name="user_name" id="user_name" ></li>
      ...
      <li>
        <div class="inputarea">
          <div class="login-input-text" id="where">驗證碼</div>
