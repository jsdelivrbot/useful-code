...
$totalRowsC = mysql_num_rows($RecNewsC);
 ...
 if (isset($_REQUEST["pageNum"])) 
 {
 	$R_pageNum = $_REQUEST["pageNum"];
	  ...
	  //若指定指定的分頁超過總分頁數則顯示最後一頁
 	  else if ($R_pageNum>$totalPages_RecNews)
      {
...
<?php
	...
	if($R_pageNum>$totalPages_RecNews && $R_pageNum!=0)
	{
...
if ($G_changeSort==1||$G_delchangeSort==1) 
{
	...
	do{
		//echo 'd_id = >'.$row_RecNews['d_id'].' d_sort =>'.$row_RecNews['d_sort'].', sort_num =>'.$sort_num.'<br>';
	...
	if($G_changeSort==1)
	{
		...
		if(isset($_GET['now_d_id'])){
			header("Location:news_list.php?selected1=".$G_selected1."&pageNum=".$_GET['pageNum']."&totalRows=".$_GET['totalRows']."#".$_GET['now_d_id']);
		}else{
			header("Location:news_list.php?selected1=".$G_selected1."&pageNum=".$_GET['pageNum']."&totalRows=".$_GET['totalRows']);
		}
		/*}else
		{
	...
	}else if($G_delchangeSort==1)
	{
		...
		{
		$G_selected2 = $_GET['selected2'];*/
			header("Location:news_list.php?selected1=".$G_selected1."&pageNum=".$_GET['pageNum']);
		/*}else
		{
			header("Location:news_list.php?selected=".$row_RecNewsC['c_id']."&pageNum=".$_GET['pageNum']);
		}*/
	}
}
...
<!--
function changeSort(selected1, pageNum, totalRows, now_d_id, change_num) { //v1.0
	alert( "news_list.php?selected1="+selected1+"&changeSort=1"+"&now_d_id="+now_d_id+"&change_num="+change_num+"&pageNum="+pageNum+"&totalRows="+totalRows );
 window.location.href="news_list.php?selected1="+selected1+"&changeSort=1"+"&now_d_id="+now_d_id+"&change_num="+change_num+"&pageNum="+pageNum+"&totalRows="+totalRows;
}
//-->
</script>
<!-- InstanceEndEditable -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
    	<tr>
        	<td width="150" class="list_title">最新消息列表</td>
...
<!-------顯示頁選擇與分頁設定結束---------->
            <td width="110" align="right" class="page_display"><?php if ($totalRows > 0) { // Show if recordset not empty ?>
                頁數:<?php echo (($pageNum+1)."/".($totalPages_RecNews+1)); ?>
	...
	<form action="news_process.php" method="post" name="form1" id="form1">
      <?php if ($totalRows > 0) { // Show if recordset not empty ?>
		...
		echo "<tr bgcolor='#E4E4E4'>";}
          ...
          <?php
		mysql_select_db($database_connect2data, $connect2data);
		$query_RecImage = "SELECT * FROM file_set  WHERE file_type='image' AND file_d_id = ".$row_RecNews['d_id'];
  ...
  <?php
        ...
        <select name="sort" id="sort" onchange="changeSort(this.options[this.selectedIndex].value, '<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_RecNews['d_id']; ?>')">
          ...
          {
          	echo "<option value=\"".$j."\" ";
			if (!(strcmp($j, $row_RecNews['d_sort']))) {echo "selected=\"selected\"";}
			echo ">".$j."</option>";
          }
		  ?>
        </select>
		...
		<?php }else{
			if (!(strcmp(0, $row_RecNews['d_sort']))) {
				echo "至頂";
			}else{
    ...
    <td align="center" class="table_data"><a href="news_edit.php?d_id=<?php echo $row_RecNews['d_id']; ?>"><img src="image/pencil.png" width="16" height="16" /></a></td>
    	<?php } ?>
            <?php if(in_array(7,$_SESSION['MM_Limit']['a3'])){ ?>
    <td align="center" class="table_data"><a href="news_del.php?d_id=<?php echo $row_RecNews['d_id']; ?>"><img src="image/cross.png" width="16" height="16" /></a></td>
    <?php } ?>
  </tr>
