...
$query_RecOrder = "SELECT O.* FROM order_set AS O LEFT JOIN order_item AS OI ON O.o_id=OI.o_id WHERE O.o_number!='' $numSQL $dateSQL $fulltxtSQL $statusSQL $itemsSQL GROUP BY O.o_id ORDER BY `datetime` DESC";
...
if (!(strcmp(1, $payment))) {$paymentTxt = "ATM 虛擬帳戶匯款";}
if (!(strcmp(2, $payment))) {$paymentTxt =  "超商店到店";}
if (!(strcmp(3, $payment))) {$paymentTxt =  "信用卡線上刷卡";}
          if(in_array(5,$_SESSION['MM_Limit']['a6'])){
...
if (!(strcmp(3, $payment))) { //線上刷卡
	...
	if($row_RecOrder['card_status']!=3){
		mysql_select_db($database_connect2data, $connect2data);
		$query_RecResponse = "SELECT ReturnStatus, RtnCode, RtnMsg, r_RtnCode, r_status FROM response_set WHERE r_lidm = '".$row_RecOrder['o_number']."' AND r_o_id = '".$row_RecOrder['o_id']."'";
		...
		if($totalRows_RecResponse>0){			
			...
			if($row_RecResponse['r_status']==-1){
				...
				/*echo 'r_status = '.$row_RecResponse['r_status'].'<br>';
				echo 'statusNote = '.$statusNote.'<br>';*/
				...
				/*if($row_RecResponse['r_RtnCode']==1){
					$r_status = 0;
				}*/
...
/*if (!(strcmp(1, $payment))) {
 ...
 if(in_array(5,$_SESSION['MM_Limit']['a6'])){
	 echo '<a href="orders_edit.php?o_id='.$row_RecOrder['o_id'].'">'.$statusNote.'</a>';
 }else{
	echo $statusNote;	 
 }
